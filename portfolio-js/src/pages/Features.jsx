const Features = () => {
  ...
  return (
    <section id="features" className="py-16 sm:py-24 bg-[#FAF7F2] border-t border-[#E8DDD0] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
          {features.map((feature, idx) => (
            <div
              key={idx}
              className="warm-card p-5 sm:p-8 rounded-2xl border border-[#E8DDD0] hover:border-[#8B5E3C] shadow-sm hover:shadow-warm transition-all duration-300 flex flex-col justify-between group"
            >
              <div>
                {/* Header Row: Code Number + Badge */}
                <div className="flex items-center justify-between pb-3 mb-3 border-b border-[#E8DDD0]">
                  <span className="font-mono text-[11px] font-bold text-[#8B5E3C] bg-[#8B5E3C]/10 px-2.5 py-0.5 rounded-md border border-[#8B5E3C]/20">
                    FEATURE // {feature.code}
                  </span>
                  <span className="w-2 h-2 rounded-full bg-[#C49A6C]" />
                </div>

                <h3 className="font-heading font-bold text-lg sm:text-xl text-[#2D1B0E] group-hover:text-[#8B5E3C] transition-colors mb-1">
                  {feature.title}
                </h3>
                <p className="font-mono text-xs font-semibold text-[#8B5E3C] uppercase tracking-wider mb-3">
                  {feature.subtitle}
                </p>

                <p className="font-sans text-xs text-[#5C4033] leading-relaxed mb-4 sm:mb-6">
                  {feature.description}
                </p>
              </div>

              {/* Bottom Supporting Tech Tags */}
              <div className="flex flex-wrap gap-1 pt-3 border-t border-[#E8DDD0]">
                {feature.tags.map((tag, tIdx) => (
                  <span
                    key={tIdx}
                    className="font-mono text-[10px] px-2 py-0.5 bg-[#FAF7F2] text-[#2D1B0E] border border-[#E8DDD0] rounded-md font-semibold"
                  >
                    {tag}
                  </span>
                ))}
